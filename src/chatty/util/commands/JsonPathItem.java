...
class JsonPathItem implements Item {
    ...
    public String replace(Parameters parameters) {
        ...
        for (Pair<Item, Boolean> subItemData : subItems) {
            ...
            if (each) {
                ...
                if (result instanceof JSONObject map) {
                ...
                } else if (result instanceof JSONArray array) {
    ...
    private record IndexItem(int index) implements PathItem {
    ...
    private record KeyItem(String key) implements PathItem {
    ...
    private record FuncItem(String key) implements PathItem {
    ...
    private record JoinItem(String delimiter) implements PathItem {
    ...
    private record FilterItem(String type, PathItem path, Pattern search) implements PathItem {

        public FilterItem(String type, PathItem path, String search) {
                this(type, path, search != null ? Pattern.compile(search) : null);
            }
    ...
    private record Parser(StringReader reader) {
    ...
    //                        error("Key expected", 0);
        ...
        private String quotedLiteral(String quote) {
                StringBuilder b = new StringBuilder();
