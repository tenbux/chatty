 ...
 * @author tduva
 */
public record DockDropInfo(DockChild dropComponent, DropType location, Rectangle rect, int index) {
    ...
    public static Rectangle makeRect(JComponent component, DropType location, int size, int otherSize, int max) {
        ...
        return switch (location) {
            case TOP -> new Rectangle(w2, 0, width - w2 * 2, h);
            case LEFT -> new Rectangle(0, h2, w, height - h2 * 2);
            case RIGHT -> new Rectangle(width - w - 1, h2, w, height - h2 * 2 - 1);
            case BOTTOM -> new Rectangle(w2, height - h - 1, width - w2 * 2, h);
            default -> new Rectangle((width - w) / 2, (height - h) / 2, w, h);
        };
    }
